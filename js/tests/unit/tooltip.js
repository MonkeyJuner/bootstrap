$(function () {
  ...
  QUnit.test('should allow to pass a custom function for sanitization', function (assert) {
    assert.expect(1)
  ...
  QUnit.test('should allow to pass aria attributes', function (assert) {
    assert.expect(1)
    ...
    var $trigger = $('<a href="#" rel="tooltip" data-trigger="click" title="Another tooltip"/>')
      ...
      .bootstrapTooltip({
        template: [
          '<span aria-pressed="true">Some contents</span>'
        ].join('')
      })
  ...
  QUnit.test('should not take in consideration sanitize in data attributes', function (assert) {
    assert.expect(1)
